...
int main(int argc, char* argv[]) {
    ...
    std::string content_arg = "-r \"" + file_content.str() + "\" >> php_output.txt";
	...
	// Récupérer la sortie de PHP et l'afficher
	std::ifstream php_output_file("php_output.txt");
	if (php_output_file.is_open()) {
	    std::stringstream result_stream;
	    result_stream << php_output_file.rdbuf();
	    std::string result = result_stream.str();
	    std::cout << result << std::endl;
	    php_output_file.close();
	} else {
	    std::cerr << "Error opening PHP output file" << std::endl;
	    return 1;
	}
